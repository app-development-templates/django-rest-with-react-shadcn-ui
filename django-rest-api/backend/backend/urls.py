...
urlpatterns = [
    ...
    path('api/user/token/refresh/', CustomTokenRefreshView.as_view(), name='refresh'),
    path('api-auth/', include('rest_framework.urls')),  # Optional: for browsable API login/logout

    # YOUR PATTERNS
    path('api/schema/', SpectacularAPIView.as_view(), name='schema'),
    path('api/schema.json', SpectacularAPIView.as_view(), kwargs={'format': 'openapi'}, name='schema-json'),
    path('api/schema.yaml', SpectacularAPIView.as_view(), kwargs={'format': 'yaml'}, name='schema-yaml'),
    # Optional UI:
